...
var page = webpage.create()
  , files = phantom.args.slice(9, phantom.args.length)
...
var options = {
      ...
      , width: width
      , pdf: phantom.args[8] === 'true' ? true : false
    }
...
files.forEach(function(file) {
  ...
  if (options.png || options.pdf) {
    ...
    if(options.png) {
      ...
      page.render(options.outputDir + fs.separator + filenameWithExt);
      console.log('Saved png: ' + filenameWithExt)
    }

    if(options.pdf) {
      filenameWithExt = filename + '.pdf';
      page.render(options.outputDir + fs.separator + filenameWithExt);
      console.log('Saved pdf: ' + filenameWithExt)
    }
  }
  console.log('After phantom render to PNG and/or PDF.');
